...
function parseNumber(v){
  ...
  // Remove currency symbols/letters and keep digits/separators/sign
  s = s.replace(/[^0-9,\.\-]/g, "");
  if(!s) return null;
  ...
  const lastComma = hasComma ? norm.lastIndexOf(",") : -1;
  const lastDot   = hasDot   ? norm.lastIndexOf(".") : -1;
  ...
  if(hasComma && hasDot){
    ...
    // If single comma and exactly 3 digits after -> thousand separator
    ...
    // If single dot and exactly 3 digits after -> thousand separator
    const parts = norm.split(".");
    if(parts.length === 2 && parts[1].length === 3){
      norm = parts[0] + parts[1];
...
function classifyRow(code, desc, qty, unit, vu, vt){
  ...
  // 1) Subtotales del Excel: NO los usamos (los generamos en la UI)
  if(
    d.startsWith("subtotal") || c.startsWith("subtotal") ||
    d.includes("subtotal") || c.includes("subtotal")
  ) return "SUBTOTAL";

  // 2) Filas de AIU / Totales (a veces vienen sin código)
  if(
    d.startsWith("administr") || d.startsWith("imprev") || d.startsWith("utilidad") ||
    d.includes("a.i.u") || d === "aiu"
  ) return "AIU";

  if(
    d.includes("valor total") || d.includes("incluye a.i.u") || d.includes("incluye aiu")
  ) return "TOTAL";

  // 3) Jerarquía numérica: CAP / SUB / ITEM
  ...
  // 4) Códigos alfanuméricos tipo "NP 1", "NP1"
  if(cn) return "ITEM";

  // 5) Si tiene datos numéricos pero no tiene código jerárquico, trátalo como ITEM
  // (OJO: si más arriba fue detectado como TOTAL/AIU, ya habrá retornado)
  if(qty !== null || unit || vu !== null || vt !== null) return "ITEM";
...
function findPlannedHeaderRow(rows){
  // Heuristic: row that contains (Fecha+Costo) OR many date-like headers (Project export)
  let best = {idx:-1, score:0};
  for(let i=0;i<Math.min(rows.length, 80);i++){
    const r = rows[i] || [];
    const text = r.map(c=> String(c??"").toLowerCase().trim()).filter(Boolean);
    ...
    const kw = ["fecha","costo","cost","comienzo","inicio","fin","duración","duracion","nombre de tarea","tarea"];
    const hits = text.reduce((acc,t)=> acc + (kw.some(k=>t.includes(k)) ? 1 : 0), 0);

    // count date-like headers (string dates or Excel serials)
    let dateCols = 0;
    for(const cell of r){
      if(toISODate(cell)) dateCols++;
    }

    const score = hits*6 + dateCols*4 + Math.min(nonEmpty, 12);
    if(score > best.score){
      best = {idx:i, score};
    }
  }
  ...
  const hasFecha = lower.some(h=>h.includes("fecha"));
  const hasCosto = lower.some(h=>h.includes("costo") || h.includes("cost"));
  let dateCols = 0;
  for(const cell of row){
    if(toISODate(cell)) dateCols++;
  }
  if((hasFecha && hasCosto) || dateCols >= 3) return best.idx;
  return -1;
}

function mapColumns(headerRow){
  const H = headerRow.map(h=> String(h??"").toLowerCase().trim());
  ...
  const col = {
    code: find(["item","ítem","codigo","código","cod","ítem."]),
    desc: find(["descripcion","descripción","desc"]),
    unit: find(["unidad","und","u."]),
    qty:  find(["cantidad","cant","qty"]),
    // unit value: soporta "Vlr. Unitario", "Valor unitario", "Vr Unit", "VU"
    vu:   find(["vlr unit","vlr. unit","vlr unitario","valor unit","valor unitario","vr unit","v/u","vu","unitario"]),
    // total value: soporta "Vlr. Total", "Valor total", "Vr Total", "VT"
    vt:   find(["vlr total","vlr. total","vlr tot","valor total","vr total","v/t","vt","total"])
  };
  return col;
}
...
function parseBudgetFromSheet(rows){
  ...
  for(let r = headerIdx+1; r<rows.length; r++){
    ...
    if(typ === "SUBTOTAL") continue; // se generan en pantalla
...
function buildBudgetDisplayRows(items, project, settings){
  const rows = [];
  const cur = project.contract.currency || "COP";
  ...
  const pushCapSubtotal = ()=>{
    if(!cap) return;
    rows.push({
      type: "SUBTOTAL",
      code: "",
      codeNorm: "",
      desc: `Subtotal ${cap.desc || cap.code || ""}`.trim(),
      unit: "",
      qtyContract: null,
      vuContract: null,
      vtContract: capBase,
  ...
  for(const it of (items||[])){
    // Al iniciar un nuevo CAP, cerramos el anterior con su subtotal
    if(it.type === "CAP"){
      pushCapSubtotal();
      cap = it;
      capBase = 0;
    ...
    if(it.type === "ITEM"){
      capBase += (it.qtyContract||0) * (it.vuContract||0);
      const eff = budgetEffectiveItem(it, project);
      capEff += eff.vt;
    }
  }
...
function renderBudget(){
  ...
  const rows = buildBudgetDisplayRows((p.budget.items||[]), p, settings);
  ...
  const body = rows.map(it=>{
    const isSummary = (it.type === "SUBTOTAL" || it.type === "AIU" || it.type === "TOTAL");
    const badge = isSummary ? "sub" : (it.type==="ITEM" ? "item" : (it.type==="CAP" ? "cap" : "sub"));

    if(isSummary){
      const vtBase = (typeof it.vtContract === "number" && isFinite(it.vtContract)) ? it.vtContract : 0;
      const vtEff = (typeof it.vtEff === "number" && isFinite(it.vtEff)) ? it.vtEff : vtBase;

      const baseCells = showBase ? `
        <td class="num"></td>
        <td class="num"></td>
        <td class="num">${formatMoney(vtBase, settings, cur)}</td>
      ` : "";

      const revCells = revs.map(()=>`
        <td class="num"></td>
        <td class="num"></td>
        <td class="num"></td>
      `).join("");
      ...
      return `
        <tr class="row-subtotal">
          <td><span class="badge ${badge}">${escapeHtml(it.type)}</span></td>
          <td>${escapeHtml(it.code||"")}</td>
          <td><b>${escapeHtml(it.desc||"")}</b></td>
          <td class="center">${escapeHtml(it.unit||"")}</td>
          ${baseCells}
          ${revCells}
          <td class="num"></td>
          <td class="num"></td>
          <td class="num"><b>${formatMoney(vtEff, settings, cur)}</b></td>
        </tr>
      `;
    }

        const extraDec = settings.highlightExtraQty && (decimalsCount(it.qtyContract) > settings.qtyDecimals);
...
function parsePlanned(rows){
  ...
  const headerIdx = findPlannedHeaderRow(rows);
  if(headerIdx < 0){
    return {curve:[], warnings:["No pude detectar columnas de fecha/costo. Intenta una hoja con columnas 'Fecha' y 'Costo', o una exportación con fechas como encabezados."]};
  }

  const headerRow = rows[headerIdx] || [];
  const headerStr = headerRow.map(h=> String(h??"").trim());
  const lower = headerStr.map(h=>h.toLowerCase());

  // Case A: columnas Fecha/Costo
  const iFecha = lower.findIndex(h=>h.includes("fecha"));
  const iCosto = lower.findIndex(h=>h.includes("costo") || h.includes("cost"));
  ...
  if(iFecha >= 0 && iCosto >= 0){
    for(let r=headerIdx+1; r<rows.length; r++){
      const row = rows[r] || [];
      const dateIso = toISODate(row[iFecha]);
      const cost = parseNumber(row[iCosto]);
      if(!dateIso || cost === null) continue;
      points.push({date: dateIso, cost});
    }
  }else{
    // Case B: fechas como encabezados (export de Project)
    const dateCols = [];
    ...
    const byDateCost = new Map();
    for(const dc of dateCols){
      byDateCost.set(dc.iso, 0);
    }
    for(let r=headerIdx+1; r<rows.length; r++){
      const row = rows[r] || [];
      for(const dc of dateCols){
        const v = parseNumber(row[dc.idx]);
        if(v === null) continue;
        byDateCost.set(dc.iso, (byDateCost.get(dc.iso)||0) + v);
      }
    }
    for(const [date, cost] of byDateCost.entries()){
      // keep zeros out to reduce noise
      if(cost !== 0) points.push({date, cost});
    }
  }

  // Normalizar y acumular (se asume que "cost" es incremental por fecha/periodo)

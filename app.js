...
function classifyRow(code, desc, qty, unit, vu, vt){
  ...
  // subtotal lines (sometimes "Subtotal ..." ends up in código o descripción)
  if(c.startsWith("subtotal") || d.startsWith("subtotal") || c.includes("subtotal") || d.includes("subtotal")) return "SUBTOTAL";

  // numeric hierarchy codes: CAP/SUB/ITEM based on level
  ...
  // alphanumeric codes like "NP 1", "NP1"
  if(cn) return "ITEM";

  // if it has any meaningful data (qty or unit or prices), treat as item
...
function renderBudget(){
  ...
  const body = rows.map(it=>{
    const badge = it.type==="ITEM" ? "item" : (it.type==="CAP" ? "cap" : "sub");

...
function classifyRow(code, desc, qty, unit, vu, vt){
  ...
  // subtotal lines
  if(d.startsWith("subtotal")) return "SUBTOTAL";
...
function parseBudgetFromSheet(rows){
  ...
  // sort items by codeNorm (numeric hierarchy first, then others)
  items.sort((a,b)=>{
    const aNum = /^\d/.test(a.codeNorm);
    const bNum = /^\d/.test(b.codeNorm);
    if(aNum && bNum) return a.codeNorm.localeCompare(b.codeNorm, "es", {numeric:true});
    if(aNum) return -1;
    if(bNum) return 1;
    return a.codeNorm.localeCompare(b.codeNorm);
  });
